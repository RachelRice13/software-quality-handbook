...
**1. White Box Testing** This sort of computer program testing assesses and confirms the 'source code', or the inner workings of a software system, such as its code and framework. White Box is an essential portion in a advanced Continuous Integration(CI)/Continuous Delivery (CD) of automated build processes.
...
![White Box Technique](../images/WhiteBox.png)

**2. Black Box Testing** Black box testing, sometimes referred to as behavioural testing, is a method of software testing in which the program is tested without being aware of its internal code structure. Only the term implies that the tester does not view the software application. This kind of testing frequently simply examines the software system's input and output.
...
![Black Box Technique](../images/BlackBox.png)

**3.Grey Box Testing**
Black box testing and white box testing methodologies are combined to create grey box testing. So while White Box Testing gives the tester unrestricted access to such knowledge, Black Box Testing prevents the tester from knowing how the programme being tested inside functions. In order to create test cases, grey box testing gives the tester access to internal data and designs as well as partial knowledge of the internal structure.
...
**Some disadvantages of Grey Box Testing are:**
-	Testers have no access to source code and may miss certain critical vulnerabilities.
-	Grey box testing may be redundant if the application developer has already run a similar test case.
-	Grey box testing is not ideal for algorithm testing.
-	Testing every potential input is too time-consuming and unrealistic, meaning certain program paths will not be tested.

![Grey Box Technique](../images/GreyBox.png)
